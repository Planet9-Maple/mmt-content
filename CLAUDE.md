...
| 컬럼 | 설명 | 저장 시점 |
|------|------|----------|
| date | 날짜 (YYYY-MM-DD) | 월간 주제 확정 시 |
| day | 요일 | 월간 주제 확정 시 |
| topic | 주제명 | 월간 주제 확정 시 |
| status | 상태 (pending/in_progress/completed) | 자동 업데이트 |
| level1_context | L1 맥락 (Claude 분석) | 구조검토 승인 시 |
| level2_context | L2 맥락 (Claude 분석) | 구조검토 승인 시 |
| level3_context | L3 맥락 (Claude 분석) | 구조검토 승인 시 |
| updated_at | 마지막 수정 시간 | 자동 |

**레벨별 맥락 형식 (출판용 인사이트):**
```
장면: 벚꽃나무 가까이 가서 꽃을 자세히 관찰하는 장면
흐름: 색깔 관찰 → 체험 유도 → 감정 확인 + 추가 체험
엄마 말 맥락:
  1️⃣ 꽃잎을 가리키며 '분홍색 꽃이 많이 피었네!'
  2️⃣ 떨어진 꽃잎을 주우며 '꽃잎이 떨어졌어, 만져볼래?'
  3️⃣ 아이와 함께 보며 '정말 예쁘지? 냄새도 맡아봐'
아이 반응:
  ⭐ 반응1: 꽃잎 만지며 반응 (2-3단어)
  ⭐ 반응2: 냄새 맡고 감탄 (2-3단어)
학습 포인트: 색깔(pink), 상태(많이, 떨어지다), 감각 동사
```

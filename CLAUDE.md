...
- `docs/mommytalk_pipeline_prompts_v2_final.md` — **전체 파이프라인 설계서 (반드시 먼저 읽을 것)**
  - Step 0~4 시스템 프롬프트 전문
  - 레벨별 생성 규칙 (단어 수, 아이 반응, 금지 표현)
  - 출력 텍스트 형식 (이모지 넘버링, {아이이름} 플레이스홀더)
  - 재생성 로직, 모델 교체 전략
- `docs/prompt_crosscheck_report.md` — DB vs 규칙 교차 점검 리포트
- `data/마미톡_컨텐츠_v_2.xlsx` — 실제 콘텐츠 DB (131건, No.290~420)
...
|---|---|---|---|
| 단어 수 | 짧은 절, 단일 절 7w 이하 | 5-10w 정도 | 8-15w |
| 구조 | 한 절. 짧은 절 2개 조합 허용 | 접속사·전치사구 허용 | 종속절·시간부사·원인결과 |
| 금지 | 접속사, 종속절 | 복문 | 어휘만 어렵게 하기 |

### 아이 반응
| | L1 | L2 | L3 |
|---|---|---|---|
| 유무 | 없음 ("생략") | 2세트 | 2세트 |
| 단어 수 | - | 2-3w chunk | 3-6w 완전문 |
| 허용 | - | "I win!" "I can do it!" 기본패턴 조건부 허용 | 조동사, to부정사, 의문문 |
| 금지 | - | 1단어, 5w+, will+동사, should, 의문문(Can I~?) | - |

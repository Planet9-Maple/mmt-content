...
3개 프로바이더: Gemini(주제) → Claude Opus(구조/생성) → GPT-5.2(검수)
...
def main():
    ...
    <div style="background: linear-gradient(90deg, #4285F4 0%, #7C3AED 33%, #10A37F 66%, #7C3AED 100%);
        ...
        🔵 Gemini (주제) → 🟣 Claude Opus (구조/생성) → 🟢 GPT-5.2 (검수) → 👩‍💻 관리자
    </div>
...
def render_gen_step1_structure_review():
    ...
    with col3:
        if st.button("✅ 승인 → 영어 생성 + AI 검수", type="primary", use_container_width=True):
            ...
            with st.spinner("🟢 GPT-5.2가 품질을 검수하고 있어요... (문제 발견 시 자동 수정)"):
                try:
...
def render_gen_step2_content_with_review():
    ...
    if not review_result:
        ...
        # 검수 자동 실행
        with st.spinner("🟢 GPT-5.2가 품질을 검수하고 있어요..."):
            try:
                topic_data = st.session_state.planned_topics[st.session_state.current_topic_idx]
        ...
        with tab:
            ...
            if st.button(
                ...
                if not target_sentences:
                    ...
                    # 2. 재검수 (자동)
                    with st.spinner(f"🟢 GPT-5.2가 재검수하고 있어요..."):
                        try:
    ...
    with col2:
        ...
        if st.button("🔄 전체 재생성 + 재검수", use_container_width=True, help="모든 레벨을 처음부터 재생성 후 자동 수정 루프 실행"):
            with st.spinner("🟣 Claude Opus가 재생성 중... 🟢 GPT-5.2 검수 + 자동 수정"):
                try:
                    # 1. 전체 재생성

...
def get_or_create_spreadsheet(spreadsheet_name: str = "마미톡잉글리시 콘텐츠 DB") -> gspread.Spreadsheet:
    ...
    try:
        ...
        # 첫 번째 시트 이름을 monthly_content로 변경하고 헤더 추가
        worksheet = spreadsheet.sheet1
        worksheet.update_title("monthly_content")
...
def append_content(
    ...
    try:
        ...
        # monthly_content 탭 사용
        try:
            worksheet = spreadsheet.worksheet("monthly_content")
        except gspread.WorksheetNotFound:
            # 탭이 없으면 생성
            worksheet = spreadsheet.add_worksheet(title="monthly_content", rows=500, cols=8)
            worksheet.update('A1:H1', [COLUMNS])
            worksheet.format('A1:H1', {
                'textFormat': {'bold': True},
                'backgroundColor': {'red': 0.9, 'green': 0.9, 'blue': 0.9}
        ...
        # 날짜순 삽입 위치 찾기
        insert_row = 2  # 헤더 다음부터 시작
        for idx, row in enumerate(all_values[1:], start=2):
            if len(row) > 1 and row[1]:
                existing_date = row[1]
                if target_date < existing_date:
                    # 현재 날짜보다 큰 날짜를 찾으면 그 앞에 삽입
                    insert_row = idx
                    break
                insert_row = idx + 1  # 현재 행 다음에 삽입

        # No. 재계산 (삽입 위치 기준)
        next_no = insert_row - 1
        ...
        # 날짜순 위치에 행 삽입
        worksheet.insert_row(new_row, insert_row, value_input_option='USER_ENTERED')

        # 삽입 후 No. 재정렬 (삽입된 행 이후의 모든 No. 업데이트)
        # 성능을 위해 일괄 업데이트
        if insert_row < len(all_values) + 1:
            updates = []
            for i in range(insert_row + 1, len(all_values) + 2):
                updates.append({'range': f'A{i}', 'values': [[i - 1]]})
            if updates:
...
def get_all_contents(spreadsheet_name: str = "마미톡잉글리시 콘텐츠 DB") -> list:
    """시트의 모든 콘텐츠를 가져옵니다 (monthly_content 탭)."""
    try:
        spreadsheet = get_or_create_spreadsheet(spreadsheet_name)
        # monthly_content 탭 사용
        try:
...
def delete_content(row_number: int, spreadsheet_name: str = "마미톡잉글리시 콘텐츠 DB") -> dict:
    ...
    try:
        ...
        try:
            ...
            return {'success': False, 'error': 'monthly_content 탭이 없습니다'}
        ...
        if deleted_date:
            upsert_topic_status(deleted_date, "", "pending", spreadsheet_name)
...
def update_content(
    ...
    """특정 행의 콘텐츠를 업데이트합니다 (monthly_content 탭)."""
    try:
        spreadsheet = get_or_create_spreadsheet(spreadsheet_name)
        # monthly_content 탭 사용
        try:
...
def add_content_raw(
    ...
    """원본 DB 형식으로 직접 콘텐츠를 추가합니다 (복구용, monthly_content 탭)."""
    try:
        spreadsheet = get_or_create_spreadsheet(spreadsheet_name)
        # monthly_content 탭 사용
        try:
...
PLANS_COLUMNS = [
    ...
    "suggestions"     # Gemini 추천 주제 목록 (JSON)
]
...
def get_or_create_plans_worksheet(spreadsheet_name: str = "마미톡잉글리시 콘텐츠 DB"):
    ...
    try:
        ...
        # 새 시트 생성 (9컬럼)
        worksheet = spreadsheet.add_worksheet(title=PLANS_SHEET_NAME, rows=500, cols=9)
        worksheet.update('A1:I1', [PLANS_COLUMNS])
...
def update_topic_suggestions(
    date: str,
    suggestions: list,
    spreadsheet_name: str = "마미톡잉글리시 콘텐츠 DB"
) -> dict:
    """특정 날짜의 suggestions를 업데이트합니다.

    Args:
        date: 날짜 (YYYY-MM-DD)
        suggestions: 추천 주제 리스트 [{"topic": "...", "reason": "..."}, ...]
        spreadsheet_name: 스프레드시트 이름
    ...
    try:
        ...
        for i, row in enumerate(all_values[1:], start=2):
            if row and row[0] == date:
                # suggestions를 JSON으로 저장
                suggestions_json = json.dumps(suggestions, ensure_ascii=False) if suggestions else ''
                worksheet.update(f'I{i}', [[suggestions_json]])
                return {'success': True, 'date': date, 'row': i}

        return {'success': False, 'error': f'날짜 {date}를 찾을 수 없습니다'}
